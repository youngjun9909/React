...
interface Todo {
  description: string; // 사용자로부터 입력받은 데이터
  completed: boolean; // 기본 값: false
    ...
    >
      ...
      <ul>
        {todos.map((todo,index) => (
          <li key={index}>
          {todo.description}
          (추가 시간: {todo.timestamp.toLocaleDateString()})

          <button onClick={() => toggleTodo(index)}>{todo.completed ? '완료 취소' : '할 일 완료'}</button>
